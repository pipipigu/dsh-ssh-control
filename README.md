...
- **100% Non-Intrusive Dual-Track Architecture**: Eliminates global hijacking of DSH native `bash`, `fs`, and `subprocess`. The local environment remains 100% native and stable; remote network disconnects never affect local sessions.
...
- **Batch Multi-Host Concurrent Dispatch**: Natively supports comma-separated targets (e.g. `server: "nas, deploy-01, 43server"`). Powered by `Promise.all` concurrent SSH pipes for parallel batch execution and aggregated reporting.
...
```bash
# Run typecheck and all 24 unit test suites (87 tests)
pnpm run check
