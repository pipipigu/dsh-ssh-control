...
export function RemoteSshSettings({ t: optionalT }: LocalizedProps): ReactElement {
  ...
  const [openFileMode, setOpenFileMode] = useState<OpenFileMode>('auto')
  ...
  return (
    <section style={{ ...page, maxWidth: 840, gap: 20 }}>
      ...
      {/* Section 2: Configuration & Discovery */}
      <div style={card}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 4 }}>
          <label htmlFor="open-file-mode-select" style={{ fontSize: 13, fontWeight: 600 }}>{t('openFileLabel')}</label>
          <select
            id="open-file-mode-select"
            style={{ ...singleLineInput, width: '100%', cursor: 'pointer' }}
            value={openFileMode}
            onChange={e => setOpenFileMode(e.target.value as OpenFileMode)}
          >
          ...
          {openFileMode === 'custom' ? (
            <>
              <label htmlFor="custom-editor-input" style={{ fontSize: 13 }}>{t('customEditorLabel')}</label>
              <input
