...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			server: {
				type: "string",
				description: "Target server ID, OpenSSH host alias (e.g. \"raydrive-nas\"), SSH target, or multiple hosts separated by commas (e.g. \"raydrive-nas, deploy-01, 43server\"). Defaults to active attached server if omitted."
			},
			command: {
				type: "string",
		...
		async execute(args, exec) {
			...
			switch (args.action) {
				...
				case "exec": {
					if (servers.length === 0) throw new Error("ssh_control exec: no server specified and no active server attached. Provide \"server\" (e.g. \"raydrive-nas\" or \"nas, deploy-01\").");
