...
本插件为 DSH 提供**跨平台兼容、智能编码自愈、支持单机直连与多机并发广播**的远程服务器操作能力，大模型可在同一个本地会话中自由调度多台远程 Linux / NAS / Windows / 容器，无需对本地开发环境造成任何侵入。
...
- **多机并发广播调度（Batch Multi-Host）**：原生支持逗号分隔多台主机（如 `server: "nas-server, app-node, web-cluster"`），底层通过 `Promise.all` 并发管道并行执行并聚合返回结果；
...
> 
> **大模型**：直接调用 `ssh_control(action: "status", server: "nas-server")`，返回内存与全部挂载点水位（`/`、`/volume1` 等）。

#### 场景 2：多机文件并发分发
> **用户**：“把本地编译好的 dist/ 目录一键上传到集群各节点的 /srv/web 路径下。”
> 
> **大模型**：调用 `ssh_control(action: "upload", server: "web-node-01, web-node-02", localPath: "dist", remotePath: "/srv/web")`。

#### 场景 3：大日志精准排障
> **用户**：“查看远程服务器上 app.log 最后的 100 行错误日志。”
> 
> **大模型**：调用 `ssh_control(action: "tail", server: "app-server", path: "/var/log/app.log", lines: 100, pattern: "ERROR|FATAL")`。
